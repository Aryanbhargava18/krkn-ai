...
def _expand_composite_json(
    ...
    if scenario.dependency == CompositeDependency.NONE:
        ...
        if isinstance(scenario_b, CompositeScenario):
            nodes_b, term_b = _expand_composite_json(
                scenario_b, key_b, depends_on=depends_on
            )
            result.update(nodes_b)
        else:
        ...
        if isinstance(scenario_a, CompositeScenario):
            nodes_a, term_a = _expand_composite_json(
                scenario_a, key_a, depends_on=term_b
            )
            result.update(nodes_a)
        else:
        ...
        if isinstance(scenario_a, CompositeScenario):
            nodes_a, term_a = _expand_composite_json(
                scenario_a, key_a, depends_on=depends_on
            )
            result.update(nodes_a)
        else:
        ...
        if isinstance(scenario_b, CompositeScenario):
            nodes_b, term_b = _expand_composite_json(
                scenario_b, key_b, depends_on=term_a
            )
            result.update(nodes_b)
        else:
